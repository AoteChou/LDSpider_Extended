...
public class HttpDownloadUtil {
    ...
    public static void downloadFile2(String fileURL){
    	HttpGet hget = new HttpGet(fileURL);
		hget.setHeaders(CrawlerConstants.HEADERS);
		
		try {
			HttpResponse hres = new HttpClient().connect(hget);
    ...
    public static void main(String[] args) {
		String uri = "http://localhost:8080/marmotta/resource?uri=http%3A%2F%2Flocalhost%2FCorrections&amp;format=application/rdf%2Bxml";

		try {
			HttpDownloadUtil.downloadFile(uri);
		} catch (IOException e) {
