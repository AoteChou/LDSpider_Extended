...
public class LoadBalancingQueue extends RedirectsFavouringSpiderQueue {
	...
	private static final Logger _log = Logger
			.getLogger(LoadBalancingQueue.class.getName());
	...
	public synchronized void schedule(Frontier f) {
		...
		_log.info("scheduling depth " + _depth + " with " + size()
				+ " uris and " + getSortedQueuePlds().size() + " plds done in "
				+ (_mintime - time) + " ms");
	}
	 ...
	 * DELAY, wait for DELAY ms to avoid overloading servers.
	 * 
	 * @return URI
	 */
		...
		do {
			...
			if (_current.isEmpty()) {
				...
				if ((time1 - _mintime) < _mindelay) {
					_log.info("fetching plds too fast, rescheduling, remaining uris in queue "
							+ size());
					_log.info(toString());
					_current = POISON;
					return null;
				...
				_log.info("skipped to start of queue in " + (time1 - _maxtime)
						+ " ms, queue size " + size());
