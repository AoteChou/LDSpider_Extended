...
public abstract class SpiderQueue implements Serializable {
	...
	private static final Logger _log = Logger.getLogger(SpiderQueue.class
			.getName());
	...
	LRUMapCache<URI, Integer> _redirsCache = new LRUMapCache<URI, Integer>(
			2 * CrawlerConstants.NB_THREADS);
	...
	/**
	 * Schedule URIs in Frontier (i.e. put URIs in Frontier into the queue for
	 * the next round)
	 */
	...
	public void setRedirect(URI from, URI to, int status) {
		...
		if (i.intValue() >= CrawlerConstants.MAX_REDIRECTS) {
			_log.info("Too many redirects on path to: " + to
					+ " ; previous on path: " + from + " .");
			return;
		}
...
//			// cend
	...
	/**
	 * Return redirected URI (if there's a redirect) otherwise return original
	 * URI.
	 * 
	 * @param from
	 * @return
